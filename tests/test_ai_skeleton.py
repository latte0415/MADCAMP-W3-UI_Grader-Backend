...
async def test_update_run_memory() -> None:
    """Playwright로 노드 생성 후 스크린샷으로 update-run-memory 실행"""
    # 테스트 URL
    target_url = os.getenv("TEST_TARGET_URL", "https://madcamp-w2-decision-maker-web.vercel.app")
    start_url = os.getenv("TEST_START_URL", "https://madcamp-w2-decision-maker-web.vercel.app/login")
    ...
    try:
        ...
        # 6. AI update-run-memory 실행
        print(f"\n5. AI update-run-memory 실행 중...")
        print(f"   Run ID: {run_id}")
        ai_service = AiService()
        response = await ai_service.update_run_memory_with_ai(
            image_base64=image_base64,
            run_id=run_id,
            auxiliary_data=auxiliary_data
        )
        ...
        if run_memory:
            ...
            print(f"   Content: {run_memory.get('content', {})}")
        else:
